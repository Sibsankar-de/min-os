...
ebr_drive_number:			db 0					; 0x00 floppy, 0x80 has use
							db 0					; reserved
...
.root_dir_after:
	...
	mov dl, [ebr_drive_number]				; dl = drive number (we saved it prev)
...
.load_kernel_loop:
	...
	add ax, 31								; first cluster = (cluster - 2) * sectors_per_cluster + kernel_cluster
	...
	mul cx
	mov cx, 2
	div cx									; ax = index of entity in FAT, dx = cluster mod 2

	mov si, buffer
	add si, ax
	mov ax, [ds:si]							; get next entry
